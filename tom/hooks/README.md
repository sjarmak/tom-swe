...
| **UserPromptSubmit** | `dist/tom/hooks/user-prompt-submit.js` | n/a (event has no matcher support) | Appends the redacted prompt to the Tier 1 session log, then consults ToM when the prompt text scores as ambiguous; injects context via `hookSpecificOutput.additionalContext` framed as background observation (never blocks the prompt) |
...
- Read the Claude Code hook payload as **JSON on stdin** (`session_id`,
  ...
  `stop_hook_active`, `prompt`, ...) via `hook-input.ts`. Parsing is
  permissive — unknown fields are preserved.
